...
class Command(BaseCommand):
    help = 'Inspect leave requests matching example criteria and print diagnostic JSON'
    ...
    def handle(self, *args, **options):
        ...
        lt_qs = LeaveType.objects.filter(name__icontains=leave_type_name)
        if lt_qs.exists():
            qs = qs.filter(leave_type__in=lt_qs)
